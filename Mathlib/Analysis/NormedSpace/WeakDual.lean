...
lemma toWeakDual_continuous : Continuous fun x' : Dual 𝕜 E => toWeakDual x' :=
...
lemma dual_norm_topology_le_weak_dual_topology :
...
lemma isClosed_image_coe_of_bounded_of_closed {s : Set (WeakDual 𝕜 E)}
...
lemma isCompact_of_bounded_of_closed [ProperSpace 𝕜] {s : Set (WeakDual 𝕜 E)}
...
lemma isClosed_image_polar_of_mem_nhds {s : Set E} (s_nhd : s ∈ 𝓝 (0 : E)) :
...
neighborhood `s` of the origin is a closed set. -/
lemma _root_.NormedSpace.Dual.isClosed_image_polar_of_mem_nhds {s : Set E}
  (s_nhd : s ∈ 𝓝 (0 : E)) :
...
lemma isCompact_polar [ProperSpace 𝕜] {s : Set E} (s_nhd : s ∈ 𝓝 (0 : E)) :
    IsCompact (polar 𝕜 s) :=
...
lemma isCompact_closedBall [ProperSpace 𝕜] (x' : Dual 𝕜 E) (r : ℝ) :
