...
lemma exists_closed_cover_approximatesLinearOn_of_hasFDerivWithinAt [SecondCountableTopology F]
...
lemma exists_partition_approximatesLinearOn_of_hasFDerivWithinAt [SecondCountableTopology F]
    (f : E → F) (s : Set E) (f' : E → E →L[ℝ] F) (hf' : ∀ x ∈ s, HasFDerivWithinAt f (f' x) s x)
    (r : (E →L[ℝ] F) → ℝ≥0) (rpos : ∀ A, r A ≠ 0) :
    ∃ (t : ℕ → Set E) (A : ℕ → E →L[ℝ] F),
...
lemma addHaar_image_le_mul_of_det_lt (A : E →L[ℝ] E) {m : ℝ≥0}
...
lemma mul_le_addHaar_image_of_lt_det (A : E →L[ℝ] E) {m : ℝ≥0}
...
lemma _root_.ApproximatesLinearOn.norm_fderiv_sub_le {A : E →L[ℝ] E} {δ : ℝ≥0}
...
lemma addHaar_image_eq_zero_of_differentiableOn_of_addHaar_eq_zero (hf : DifferentiableOn ℝ f s)
    (hs : μ s = 0) : μ (f '' s) = 0 := by
...
lemma addHaar_image_eq_zero_of_det_fderivWithin_eq_zero_aux
...
/-- A version of Sard lemma in fixed dimension: given a differentiable function from `E` to `E` and
a set where the differential is not invertible, then the image of this set has zero measure. -/
lemma addHaar_image_eq_zero_of_det_fderivWithin_eq_zero
...
lemma aemeasurable_fderivWithin (hs : MeasurableSet s)
...
lemma aemeasurable_ofReal_abs_det_fderivWithin (hs : MeasurableSet s)
...
lemma aemeasurable_toNNReal_abs_det_fderivWithin (hs : MeasurableSet s)
    (hf' : ∀ x ∈ s, HasFDerivWithinAt f (f' x) s x) :
...
lemma measurable_image_of_fderivWithin (hs : MeasurableSet s)
...
lemma measurableEmbedding_of_fderivWithin (hs : MeasurableSet s)
...
lemma addHaar_image_le_lintegral_abs_det_fderiv_aux1 (hs : MeasurableSet s)
...
lemma addHaar_image_le_lintegral_abs_det_fderiv_aux2 (hs : MeasurableSet s) (h's : μ s ≠ ∞)
    (hf' : ∀ x ∈ s, HasFDerivWithinAt f (f' x) s x) :
...
lemma addHaar_image_le_lintegral_abs_det_fderiv (hs : MeasurableSet s)
    (hf' : ∀ x ∈ s, HasFDerivWithinAt f (f' x) s x) :
    μ (f '' s) ≤ ∫⁻ x in s, ENNReal.ofReal |(f' x).det| ∂μ := by
...
lemma lintegral_abs_det_fderiv_le_addHaar_image_aux1 (hs : MeasurableSet s)
...
lemma lintegral_abs_det_fderiv_le_addHaar_image_aux2 (hs : MeasurableSet s) (h's : μ s ≠ ∞)
    (hf' : ∀ x ∈ s, HasFDerivWithinAt f (f' x) s x) (hf : InjOn f s) :
...
lemma lintegral_abs_det_fderiv_le_addHaar_image (hs : MeasurableSet s)
    (hf' : ∀ x ∈ s, HasFDerivWithinAt f (f' x) s x) (hf : InjOn f s) :
    (∫⁻ x in s, ENNReal.ofReal |(f' x).det| ∂μ) ≤ μ (f '' s) := by
  /- We already know the result for finite-measure sets. We cover `s` by finite-measure sets using
    `spanningSets μ`, and apply the previous result to each of these parts. -/
...
lemma lintegral_abs_det_fderiv_eq_addHaar_image (hs : MeasurableSet s)
    (hf' : ∀ x ∈ s, HasFDerivWithinAt f (f' x) s x) (hf : InjOn f s) :
...
-/
lemma map_withDensity_abs_det_fderiv_eq_addHaar (hs : MeasurableSet s)
...
-/
lemma restrict_map_withDensity_abs_det_fderiv_eq_addHaar (hs : MeasurableSet s)
    (hf' : ∀ x ∈ s, HasFDerivWithinAt f (f' x) s x) (hf : InjOn f s) :
...
Note that the measurability of `f '' s` is given by `measurable_image_of_fderivWithin`. -/
lemma lintegral_image_eq_lintegral_abs_det_fderiv_mul (hs : MeasurableSet s)
...
integrable on `s`. -/
lemma integrableOn_image_iff_integrableOn_abs_det_fderiv_smul (hs : MeasurableSet s)
...
lemma integral_image_eq_integral_abs_det_fderiv_smul [CompleteSpace F] (hs : MeasurableSet s)
    (hf' : ∀ x ∈ s, HasFDerivWithinAt f (f' x) s x) (hf : InjOn f s) (g : E → F) :
...
lemma det_one_smulRight {𝕜 : Type*} [NormedField 𝕜] (v : 𝕜) :
...
`s`. -/
lemma integrableOn_image_iff_integrableOn_abs_deriv_smul {s : Set ℝ} {f : ℝ → ℝ} {f' : ℝ → ℝ}
...
lemma integral_image_eq_integral_abs_deriv_smul {s : Set ℝ} {f : ℝ → ℝ} {f' : ℝ → ℝ}
...
lemma integral_target_eq_integral_abs_det_fderiv_smul [CompleteSpace F] {f : LocalHomeomorph E E}
