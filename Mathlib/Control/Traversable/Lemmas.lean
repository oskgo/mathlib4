...
lemma map_traverse (x : t α) : map f <$> traverse g x = traverse (map f ∘ g) x := by
...
lemma traverse_map (f : β → F γ) (g : α → β) (x : t α) :
...
lemma pure_traverse (x : t α) : traverse pure x = (pure x : F (t α)) := by
...
lemma id_sequence (x : t α) : sequence (f := Id) (pure <$> x) = pure x := by
...
lemma naturality' (η : ApplicativeTransformation F G) (x : t (F α)) :
...
lemma naturality_pf (η : ApplicativeTransformation F G) (f : α → F β) :
