...
lemma mem_adjoin_support (f : R[M]) : f ∈ adjoin R (of' R M '' f.support) := by
...
lemma support_gen_of_gen {S : Set R[M]} (hS : Algebra.adjoin R S = ⊤) :
...
lemma exists_finset_adjoin_eq_top [h : FiniteType R R[M]] :
...
lemma fg_of_finiteType [CommRing R] [Nontrivial R] [h : FiniteType R R[M]] :
    AddMonoid.FG M :=
