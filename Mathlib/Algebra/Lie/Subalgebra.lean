...
lemma coe_injective : Function.Injective ((↑) : LieSubalgebra R L → Set L) :=
  SetLike.coe_injective
#align lie_subalgebra.coe_injective LieSubalgebra.coe_injective

@[norm_cast]
lemma coe_set_eq (L₁' L₂' : LieSubalgebra R L) : (L₁' : Set L) = L₂' ↔ L₁' = L₂' :=
  SetLike.coe_set_eq
#align lie_subalgebra.coe_set_eq LieSubalgebra.coe_set_eq

lemma to_submodule_injective : Function.Injective ((↑) : LieSubalgebra R L → Submodule R L) :=
  fun L₁' L₂' h ↦ by
...
@[simp]
lemma coe_to_submodule_eq_iff (L₁' L₂' : LieSubalgebra R L) :
...
lemma coe_to_submodule : ((L' : Submodule R L) : Set L) = L' :=
  rfl
...
@[simp]
lemma coe_bracket_of_module (x : L') (m : M) : ⁅x, m⁆ = ⁅(x : L), m⁆ :=
  rfl
...
@[simp]
lemma _root_.LieModuleHom.coe_restrictLie (f : M →ₗ⁅R,L⁆ N) : ⇑(f.restrictLie L') = f :=
  rfl
...
@[simp]
lemma range_coe : (f.range : Set L₂) = Set.range f :=
  LinearMap.range_coe (f : L →ₗ[R] L₂)
#align lie_hom.range_coe LieHom.range_coe

@[simp]
lemma mem_range (x : L₂) : x ∈ f.range ↔ ∃ y : L, f y = x :=
  LinearMap.mem_range
#align lie_hom.mem_range LieHom.mem_range

lemma mem_range_self (x : L) : f x ∈ f.range :=
...
@[simp]
lemma rangeRestrict_apply (x : L) : f.rangeRestrict x = ⟨f x, f.mem_range_self x⟩ :=
  rfl
#align lie_hom.range_restrict_apply LieHom.rangeRestrict_apply

lemma surjective_rangeRestrict : Function.Surjective f.rangeRestrict := by
  rintro ⟨y, hy⟩
...
@[simp]
lemma equivRangeOfInjective_apply (h : Function.Injective f) (x : L) :
...
lemma Submodule.exists_lieSubalgebra_coe_eq_iff (p : Submodule R L) :
...
@[simp]
lemma incl_range : K.incl.range = K := by
...
lemma le_def : K ≤ K' ↔ (K : Set L) ⊆ K' :=
  Iff.rfl
#align lie_subalgebra.le_def LieSubalgebra.le_def

@[simp]
lemma coe_submodule_le_coe_submodule : (K : Submodule R L) ≤ K' ↔ K ≤ K' :=
  Iff.rfl
...
@[simp]
lemma bot_coe : ((⊥ : LieSubalgebra R L) : Set L) = {0} :=
  rfl
#align lie_subalgebra.bot_coe LieSubalgebra.bot_coe

@[simp]
lemma bot_coe_submodule : ((⊥ : LieSubalgebra R L) : Submodule R L) = ⊥ :=
  rfl
#align lie_subalgebra.bot_coe_submodule LieSubalgebra.bot_coe_submodule

@[simp]
lemma mem_bot (x : L) : x ∈ (⊥ : LieSubalgebra R L) ↔ x = 0 :=
  mem_singleton_iff
...
@[simp]
lemma top_coe : ((⊤ : LieSubalgebra R L) : Set L) = univ :=
  rfl
#align lie_subalgebra.top_coe LieSubalgebra.top_coe

@[simp]
lemma top_coe_submodule : ((⊤ : LieSubalgebra R L) : Submodule R L) = ⊤ :=
  rfl
#align lie_subalgebra.top_coe_submodule LieSubalgebra.top_coe_submodule

@[simp]
lemma mem_top (x : L) : x ∈ (⊤ : LieSubalgebra R L) :=
  mem_univ x
#align lie_subalgebra.mem_top LieSubalgebra.mem_top

lemma _root_.LieHom.range_eq_map : f.range = map f ⊤ := by
  ext
  simp
...
@[simp]
lemma inf_coe : (↑(K ⊓ K') : Set L) = (K : Set L) ∩ (K' : Set L) :=
  rfl
#align lie_subalgebra.inf_coe LieSubalgebra.inf_coe

@[simp]
lemma sInf_coe_to_submodule (S : Set (LieSubalgebra R L)) :
...
@[simp]
lemma sInf_coe (S : Set (LieSubalgebra R L)) : (↑(sInf S) : Set L) = ⋂ s ∈ S, (s : Set L) := by
  rw [← coe_to_submodule, sInf_coe_to_submodule, Submodule.sInf_coe]
  ext x
  simp
#align lie_subalgebra.Inf_coe LieSubalgebra.sInf_coe

lemma sInf_glb (S : Set (LieSubalgebra R L)) : IsGLB S (sInf S) := by
...
@[simp]
lemma mem_inf (x : L) : x ∈ K ⊓ K' ↔ x ∈ K ∧ x ∈ K' := by
...
lemma eq_bot_iff : K = ⊥ ↔ ∀ x : L, x ∈ K → x = 0 := by
...
@[simp]
lemma coe_homOfLe (x : K) : (homOfLe h x : L) = x :=
  rfl
#align lie_subalgebra.coe_hom_of_le LieSubalgebra.coe_homOfLe

lemma homOfLe_apply (x : K) : homOfLe h x = ⟨x.1, h x.2⟩ :=
  rfl
...
@[simp]
lemma mem_ofLe (x : K') : x ∈ ofLe h ↔ (x : L) ∈ K := by
...
lemma ofLe_eq_comap_incl : ofLe h = K.comap K'.incl := by
  ext
  rw [mem_ofLe]
  rfl
...
@[simp]
lemma coe_ofLe : (ofLe h : Submodule R K') = LinearMap.range (Submodule.ofLe h) :=
  rfl
...
@[simp]
lemma equivOfLe_apply (x : K) : equivOfLe h x = ⟨homOfLe h x, (homOfLe h).mem_range_self x⟩ :=
  rfl
#align lie_subalgebra.equiv_of_le_apply LieSubalgebra.equivOfLe_apply

end NestedSubalgebras

lemma map_le_iff_le_comap {K : LieSubalgebra R L} {K' : LieSubalgebra R L₂} :
...
lemma mem_lieSpan {x : L} : x ∈ lieSpan R L s ↔ ∀ K : LieSubalgebra R L, s ⊆ K → x ∈ K := by
...
lemma lieSpan_le {K} : lieSpan R L s ≤ K ↔ s ⊆ K := by
  constructor
...
@[simp]
lemma ofInjective_apply (f : L₁ →ₗ⁅R⁆ L₂) (h : Function.Injective f) (x : L₁) :
...
@[simp]
lemma ofEq_apply (L L' : LieSubalgebra R L₁) (h : (L : Set L₁) = L') (x : L) :
    (↑(ofEq L L' h x) : L₁) = x :=
  rfl
...
@[simp]
lemma lieSubalgebraMap_apply (x : L₁'') : ↑(e.lieSubalgebraMap _ x) = e x :=
  rfl
...
@[simp]
lemma ofSubalgebras_apply (h : L₁'.map ↑e = L₂') (x : L₁') : ↑(e.ofSubalgebras _ _ h x) = e x :=
  rfl
#align lie_equiv.of_subalgebras_apply LieEquiv.ofSubalgebras_apply

@[simp]
lemma ofSubalgebras_symm_apply (h : L₁'.map ↑e = L₂') (x : L₂') :
