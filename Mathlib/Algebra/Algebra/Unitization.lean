...
lemma ind {R A} [AddZeroClass R] [AddZeroClass A] {P : Unitization R A → Prop}
...
working with `R × A`. -/
lemma linearMap_ext {N} [Semiring S] [AddCommMonoid R] [AddCommMonoid A] [AddCommMonoid N]
...
lemma algHom_ext {F : Type*} [AlgHomClass F S (Unitization R A) B] {φ ψ : F}
    (h : ∀ a : A, φ a = ψ a)
...
@[ext 1100]
lemma algHom_ext' {φ ψ : Unitization R A →ₐ[R] C}
    (h :
...
/-- See note [partially-applied ext lemmas] -/
@[ext]
lemma starAlgHom_ext {φ ψ : Unitization R A →⋆ₐ[R] C}
