...
lemma not_cancels_of_cons_hyp (u : ℤˣ) (w : NormalWord d)
    (h2 : ∀ u' ∈ Option.map Prod.fst w.toList.head?,
      w.head ∈ toSubgroup A B u → u = u') :
    ¬ Cancels u w := by
...
lemma map_fst_eq_and_of_prod_eq {w₁ w₂ : ReducedWord G A B}
...
occurences of `t`.  -/
lemma toList_eq_nil_of_mem_of_range (w : ReducedWord G A B)
