...
@[to_additive (attr := simp)]
lemma mem_map {f : F} {S : Submonoid M} {y : N} : y ∈ S.map f ↔ ∃ x ∈ S, f x = y := by
  rw [← bex_def]
...
lemma map_mclosure (f : F) (s : Set M) : (closure s).map f = closure (f '' s) :=
  le_antisymm
...
lemma bot_or_nontrivial (S : Submonoid M) : S = ⊥ ∨ Nontrivial S := by
...
@[to_additive]
lemma smul_def [SMul M' α] {S : Submonoid M'} (g : S) (m : α) : g • m = (g : M') • m :=
  rfl
