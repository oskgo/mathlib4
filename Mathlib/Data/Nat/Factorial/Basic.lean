...
lemma factorial_lt (hn : 0 < n) : n ! < m ! ↔ n < m := by
...
lemma factorial_eq_one : n ! = 1 ↔ n ≤ 1 := by
...
lemma factorial_inj (hn : 1 < n !) : n ! = m ! ↔ n = m := by
...
lemma lt_factorial_self {n : ℕ} (hi : 3 ≤ n) : n < n ! := by
...
lemma add_factorial_le_factorial_add (i : ℕ) {n : ℕ} (n1 : 1 ≤ n) : i + n ! ≤ (i + n)! := by
  cases' n1 with h
  · exact self_le_factorial _
  exact add_factorial_succ_le_factorial_add_succ i h
#align nat.add_factorial_le_factorial_add Nat.add_factorial_le_factorial_add

lemma factorial_mul_pow_sub_le_factorial {n m : ℕ} (hnm : n ≤ m) : n ! * n ^ (m - n) ≤ m ! := by
...
@[simp]
lemma ascFactorial_zero (n : ℕ) : n.ascFactorial 0 = 1 :=
  rfl
#align nat.asc_factorial_zero Nat.ascFactorial_zero

@[simp]
lemma zero_ascFactorial (k : ℕ) : (0 : ℕ).ascFactorial k = k ! := by
  induction' k with t ht
  · rfl
  rw [ascFactorial, ht, zero_add, Nat.factorial_succ]
#align nat.zero_asc_factorial Nat.zero_ascFactorial

lemma ascFactorial_succ {n k : ℕ} : n.ascFactorial k.succ = (n + k + 1) * n.ascFactorial k :=
  rfl
...
-- Porting note: Explicit arguments are required to show that the recursion terminates
-- Porting note: Interconversion between `succ` and `· + 1` has to be done manually
lemma factorial_mul_ascFactorial (n : ℕ) : ∀ k, n ! * n.ascFactorial k = (n + k)!
...
lemma ascFactorial_eq_div (n k : ℕ) : n.ascFactorial k = (n + k)! / n ! := by
...
lemma ascFactorial_of_sub {n k : ℕ} (h : k < n) :
...
lemma pow_succ_le_ascFactorial (n : ℕ) : ∀ k : ℕ, (n + 1) ^ k ≤ n.ascFactorial k
...
lemma pow_lt_ascFactorial' (n k : ℕ) : (n + 1) ^ (k + 2) < n.ascFactorial (k + 2) := by
...
lemma pow_lt_ascFactorial (n : ℕ) : ∀ {k : ℕ}, 2 ≤ k → (n + 1) ^ k < n.ascFactorial k
  | 0 => by rintro ⟨⟩
  | 1 => by intro; contradiction
  | k + 2 => fun _ => pow_lt_ascFactorial' n k
#align nat.pow_lt_asc_factorial Nat.pow_lt_ascFactorial

lemma ascFactorial_le_pow_add (n : ℕ) : ∀ k : ℕ, n.ascFactorial k ≤ (n + k) ^ k
  | 0 => by rw [ascFactorial_zero, pow_zero]
  | k + 1 => by
...
lemma ascFactorial_lt_pow_add (n : ℕ) : ∀ {k : ℕ}, 2 ≤ k → n.ascFactorial k < (n + k) ^ k
  | 0 => by rintro ⟨⟩
  | 1 => by intro; contradiction
  | k + 2 => fun _ => by
...
lemma ascFactorial_pos (n k : ℕ) : 0 < n.ascFactorial k :=
...
@[simp]
lemma descFactorial_zero (n : ℕ) : n.descFactorial 0 = 1 :=
  rfl
#align nat.desc_factorial_zero Nat.descFactorial_zero

@[simp]
lemma descFactorial_succ (n k : ℕ) : n.descFactorial k.succ = (n - k) * n.descFactorial k :=
  rfl
#align nat.desc_factorial_succ Nat.descFactorial_succ

lemma zero_descFactorial_succ (k : ℕ) : (0 : ℕ).descFactorial k.succ = 0 := by
...
-/
-- @[simp]
lemma descFactorial_one (n : ℕ) : n.descFactorial 1 = n := by
...
-/
-- @[simp]
lemma succ_descFactorial_succ (n : ℕ) :
...
lemma descFactorial_self : ∀ n : ℕ, n.descFactorial n = n !
...
@[simp]
lemma descFactorial_eq_zero_iff_lt {n : ℕ} : ∀ {k : ℕ}, n.descFactorial k = 0 ↔ n < k
...
lemma add_descFactorial_eq_ascFactorial (n : ℕ) :
...
lemma factorial_mul_descFactorial : ∀ {n k : ℕ}, k ≤ n → (n - k)! * n.descFactorial k = n !
...
lemma descFactorial_eq_div {n k : ℕ} (h : k ≤ n) : n.descFactorial k = n ! / (n - k)! := by
...
lemma pow_sub_le_descFactorial (n : ℕ) : ∀ k : ℕ, (n + 1 - k) ^ k ≤ n.descFactorial k
...
lemma pow_sub_lt_descFactorial' {n : ℕ} :
...
lemma descFactorial_le_pow (n : ℕ) : ∀ k : ℕ, n.descFactorial k ≤ n ^ k
  | 0 => by rw [descFactorial_zero, pow_zero]
  | k + 1 => by
...
lemma descFactorial_lt_pow {n : ℕ} (hn : 1 ≤ n) : ∀ {k : ℕ}, 2 ≤ k → n.descFactorial k < n ^ k
  | 0 => by rintro ⟨⟩
  | 1 => by intro; contradiction
  | k + 2 => fun _ => by
