...
lemma multiplicity_choose' {p n k b : ℕ} (hp : p.Prime) (hnb : log p (n + k) < b) :
...
lemma multiplicity_le_multiplicity_choose_add {p : ℕ} (hp : p.Prime) :
...
lemma multiplicity_two_factorial_lt : ∀ {n : ℕ} (_ : n ≠ 0), multiplicity 2 n ! < n := by
