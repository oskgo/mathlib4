...
`s` and `i ∉ s`. -/
lemma monotone_iff {β : Type*} [Preorder β] (f : Finset α → β) :
...
all `s` and `i ∉ s`. -/
lemma strictMono_iff {β : Type*} [Preorder β] (f : Finset α → β) :
...
`s` and `i ∉ s`. -/
lemma antitone_iff {β : Type*} [Preorder β] (f : Finset α → β) :
...
all `s` and `i ∉ s`. -/
lemma strictAnti_iff {β : Type*} [Preorder β] (f : Finset α → β) :
