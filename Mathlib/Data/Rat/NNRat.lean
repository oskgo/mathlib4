...
@[norm_cast]
lemma coe_mk (q : ℚ) (hq) : ((⟨q, hq⟩ : ℚ≥0) : ℚ) = q :=
  rfl
...
@[simp, norm_cast]
lemma coe_zero : ((0 : ℚ≥0) : ℚ) = 0 :=
  rfl
#align nnrat.coe_zero NNRat.coe_zero

@[simp, norm_cast]
lemma coe_one : ((1 : ℚ≥0) : ℚ) = 1 :=
  rfl
...
@[simp, norm_cast]
lemma coe_inv (q : ℚ≥0) : ((q⁻¹ : ℚ≥0) : ℚ) = (q : ℚ)⁻¹ :=
  rfl
#align nnrat.coe_inv NNRat.coe_inv

@[simp, norm_cast]
lemma coe_div (p q : ℚ≥0) : ((p / q : ℚ≥0) : ℚ) = p / q :=
  rfl
...
@[simp, norm_cast]
lemma coe_natCast (n : ℕ) : (↑(↑n : ℚ≥0) : ℚ) = n :=
  map_natCast coeHom n
#align nnrat.coe_nat_cast NNRat.coe_natCast

@[simp]
lemma mk_coe_nat (n : ℕ) : @Eq ℚ≥0 (⟨(n : ℚ), n.cast_nonneg⟩ : ℚ≥0) n :=
...
@[simp, norm_cast]
lemma coe_pow (q : ℚ≥0) (n : ℕ) : (↑(q ^ n) : ℚ) = (q : ℚ) ^ n :=
  coeHom.map_pow _ _
...
@[simp]
lemma natAbs_num_coe : (q : ℚ).num.natAbs = q.num :=
  rfl
#align nnrat.nat_abs_num_coe NNRat.natAbs_num_coe

@[simp]
lemma den_coe : (q : ℚ).den = q.den :=
  rfl
