...
instance instOrderBotOfUnique [Preorder α] [Unique α] : OrderBot α where
  bot := default
  bot_le a := by aesop

instance instOrderTopOfUnique [Preorder α] [Unique α] : OrderTop α where
  top := default
  le_top a := by aesop
...
def implMaxDepth (maxSize : Option Nat) (maxDepth : Option Nat) (f : α → ListM m α) (a : α) : ListM m α :=
  ...
  | some max =>
    let f' : α ×ₗ Nat → ListM m (α × Nat) := fun ⟨a, n⟩ => if max < n then nil else (f a).map fun a' => (a', n + 1)
...
instance instLocallyFiniteOrderOfUnique [Preorder α] [Unique α] : LocallyFiniteOrder α where
...
instance instLocallyFiniteOrderBotOfUnique [Preorder α] [Unique α] : LocallyFiniteOrderBot α where
  finsetIio _ := ∅
  finsetIic _ := {default}
  finset_mem_Iio := by aesop
  finset_mem_Iic := by aesop

instance instLocallyFiniteOrderTopOfUnique [Preorder α] [Unique α] : LocallyFiniteOrderTop α where
  finsetIoi _ := ∅
  finsetIci _ := {default}
