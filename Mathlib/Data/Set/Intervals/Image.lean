...
lemma Monotone.mapsTo_Icc (h : Monotone f) :
    MapsTo f (Icc a b) (Icc (f a) (f b)) :=
  fun _ _ => by aesop

lemma StrictMono.mapsTo_Ioo (h : StrictMono f) :
    MapsTo f (Ioo a b) (Ioo (f a) (f b)) :=
  fun _ _ => by aesop

lemma Monotone.mapsTo_Ici  (h : Monotone f) :
    MapsTo f (Ici a) (Ici (f a)) :=
  fun _ _ => by aesop

lemma Monotone.mapsTo_Iic (h : Monotone f) :
    MapsTo f (Iic a) (Iic (f a)) :=
  fun _ _ => by aesop

lemma StrictMono.mapsTo_Ioi (h : StrictMono f) :
    MapsTo f (Ioi a) (Ioi (f a)) :=
  fun _ _ => by aesop

lemma StrictMono.mapsTo_Iio (h : StrictMono f) :
...
lemma StrictMono.mapsTo_Ico (h : StrictMono f) :
...
lemma StrictMono.mapsTo_Ioc (h : StrictMono f) :
...
lemma Monotone.image_Icc_subset (h : Monotone f) :
    f '' Icc a b ⊆ Icc (f a) (f b) :=
  h.mapsTo_Icc.image_subset

lemma StrictMono.image_Ioo_subset (h : StrictMono f) :
    f '' Ioo a b ⊆ Ioo (f a) (f b) :=
  h.mapsTo_Ioo.image_subset

lemma Monotone.image_Ici_subset (h : Monotone f) :
    f '' Ici a ⊆ Ici (f a) :=
  h.mapsTo_Ici.image_subset

lemma Monotone.image_Iic_subset (h : Monotone f) :
    f '' Iic a ⊆ Iic (f a) :=
  h.mapsTo_Iic.image_subset

lemma StrictMono.image_Ioi_subset (h : StrictMono f) :
    f '' Ioi a ⊆ Ioi (f a) :=
  h.mapsTo_Ioi.image_subset

lemma StrictMono.image_Iio_subset (h : StrictMono f) :
    f '' Iio a ⊆ Iio (f a) :=
...
lemma StrictMono.imageIco_subset (h : StrictMono f) :
    f '' Ico a b ⊆ Ico (f a) (f b) :=
  h.mapsTo_Ico.image_subset

lemma StrictMono.imageIoc_subset (h : StrictMono f) :
