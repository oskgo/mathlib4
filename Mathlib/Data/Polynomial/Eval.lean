...
@[simp]
lemma eval₂_nat_cast (n : ℕ) : (n : R[X]).eval₂ f x = n := by
...
@[simp]
lemma eval₂_at_nat_cast {S : Type*} [Semiring S] (f : R →+* S) (n : ℕ) :
...
@[simp]
lemma eval_nat_cast {n : ℕ} : (n : R[X]).eval x = n := by simp only [← C_eq_nat_cast, eval_C]
...
-/
lemma eval_monomial_one_add_sub [CommRing S] (d : ℕ) (y : S) :
...
@[simp]
lemma eval_nat_cast_mul {n : ℕ} : ((n : R[X]) * p).eval x = n * p.eval x := by
...
@[simp]
lemma comp_C : p.comp (C a) = C (p.eval a) := by simp [comp, (C : R →+* _).map_sum]
...
@[simp]
lemma nat_cast_comp {n : ℕ} : (n : R[X]).comp p = n := by rw [← C_eq_nat_cast, C_comp]
...
@[simp]
protected lemma map_nat_cast (n : ℕ) : (n : R[X]).map f = n :=
...
@[simp]
lemma eval_int_cast_map {R S : Type*} [Ring R] [Ring S] (f : R →+* S) (p : R[X]) (i : ℤ) :
...
-/
lemma eval_list_prod (l : List R[X]) (x : R) : eval x l.prod = (l.map (eval x)).prod :=
...
-/
lemma eval_multiset_prod (s : Multiset R[X]) (x : R) : eval x s.prod = (s.map (eval x)).prod :=
...
@[simp]
lemma map_int_cast {S} [Ring S] (f : R →+* S) (n : ℤ) : map f ↑n = ↑n :=
  map_intCast (mapRingHom f) n
#align polynomial.map_int_cast Polynomial.map_int_cast

@[simp]
lemma eval_int_cast {n : ℤ} {x : R} : (n : R[X]).eval x = n := by
...
@[simp]
lemma eval₂_at_int_cast {S : Type*} [Ring S] (f : R →+* S) (n : ℤ) :
    p.eval₂ f n = f (p.eval n) := by
  convert eval₂_at_apply (p := p) f n
  simp

lemma mul_X_sub_int_cast_comp {n : ℕ} :
    (p * (X - (n : R[X]))).comp q = p.comp q * (q - n) := by
