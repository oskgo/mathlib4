...
lemma tendstoUniformlyOnFilter_iff {F : ι → β → α} {f : β → α} {p : Filter ι} {p' : Filter β} :
...
lemma tendstoLocallyUniformlyOn_iff [TopologicalSpace β] {F : ι → β → α} {f : β → α}
    {p : Filter ι} {s : Set β} :
...
lemma tendstoUniformlyOn_iff {F : ι → β → α} {f : β → α} {p : Filter ι} {s : Set β} :
...
lemma tendstoLocallyUniformly_iff [TopologicalSpace β] {F : ι → β → α} {f : β → α}
    {p : Filter ι} :
...
/-- Expressing uniform convergence using `dist`. -/
lemma tendstoUniformly_iff {F : ι → β → α} {f : β → α} {p : Filter ι} :
...
lemma eventually_nhds_prod_iff {f : Filter ι} {x₀ : α} {p : α × ι → Prop} :
...
lemma isBounded_closedBall : IsBounded (closedBall x r) :=
...
lemma _root_.TotallyBounded.isBounded {s : Set α} (h : TotallyBounded s) : IsBounded s :=
...
lemma _root_.IsCompact.isBounded {s : Set α} (h : IsCompact s) : IsBounded s :=
...
lemma isBounded_range_iff {f : β → α} : IsBounded (range f) ↔ ∃ C, ∀ x y, dist (f x) (f y) ≤ C :=
...
lemma isBounded_of_compactSpace [CompactSpace α] : IsBounded s :=
...
lemma exists_isOpen_isBounded_image_inter_of_isCompact_of_forall_continuousWithinAt
...
lemma exists_isOpen_isBounded_image_of_isCompact_of_forall_continuousAt [TopologicalSpace β]
...
lemma exists_isOpen_isBounded_image_inter_of_isCompact_of_continuousOn [TopologicalSpace β]
...
some open neighborhood of `k`. -/
lemma exists_isOpen_isBounded_image_of_isCompact_of_continuousOn [TopologicalSpace β]
...
lemma isCompact_of_isClosed_isBounded [ProperSpace α] (hc : IsClosed s) (hb : IsBounded s) :
    IsCompact s := by
...
lemma _root_.Bornology.IsBounded.isCompact_closure [ProperSpace α] (h : IsBounded s) :
    IsCompact (closure s) :=
...
lemma isBounded_of_bddAbove_of_bddBelow {s : Set α} (h₁ : BddAbove s) (h₂ : BddBelow s) :
    IsBounded s :=
  let ⟨u, hu⟩ := h₁
  let ⟨l, hl⟩ := h₂
...
lemma isBounded_iff_ediam_ne_top : IsBounded s ↔ EMetric.diam s ≠ ⊤ :=
...
/-- The distance between two points in a set is controlled by the diameter of the set. -/
lemma dist_le_diam_of_mem (h : IsBounded s) (hx : x ∈ s) (hy : y ∈ s) : dist x y ≤ diam s :=
...
lemma diam_eq_zero_of_unbounded (h : ¬IsBounded s) : diam s = 0 := by
...
lemma diam_mono {s t : Set α} (h : s ⊆ t) (ht : IsBounded t) : diam s ≤ diam t :=
...
lemma finite_isBounded_inter_isClosed [ProperSpace α] {K s : Set α} [DiscreteTopology s]
